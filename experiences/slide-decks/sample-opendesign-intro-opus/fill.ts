 ...
 * `/demo/opendesign-intro-opus`.
 *
 * DURABLE OPUS COPY: the shared `sample-opendesign-intro/` folder is being
 * rewritten by concurrent sessions, so this is the stable copy owned by this
 * (Opus) session, wired to its own route. Content is the verified compose
 * output — `FILL_SCHEMA.parse` + the real `validate_fill` domain path both pass
 * (`valid: true`, 0 findings), and all ten slides were re-shot from a fresh
 * build (evidence: docs/superpowers/specs/opendesign-intro-opus-sample/).
 *
...
export const openDesignIntroOpusFill: CircuitDeckFill = FILL_SCHEMA.parse({
  deck: {
    code: 'OD-01',
    world: 'OPENDESIGN',

/**
 * Content pack for "The Quarter" — the live rendering of
 * `deck-quarterly-business-review`.
 *
 * THE WORLD: deliberately CONVENTIONAL slide anatomy — the quarterly business
 * review a competent operator makes in PowerPoint, executed flawlessly. A
 * persistent title bar (deck title left, section right), a content zone on a
 * visible 12-column grid, a footer rule with page number + confidentiality line
 * + synthetic notice, a numbered agenda, and restrained single fade/rise motion.
 * NO world conceit, NO atmospheric field. The craft is typographic scale
 * (Fraunces display / Inter text / IBM Plex Mono figures), perfect alignment,
 * and considered charts. Accent: corporate navy.
 *
 * Anomaly (verbatim): the KPI row flags `NRR 96% — BELOW 100% FLOOR` — the
 * single red figure in an otherwise green row, echoed in the executive summary.
 *
 * Every figure is a synthetic result (declared in DECK.dataNotice).
 */
...
export const DECK = {
  ...
  subtitle: 'Q3 FY26 · Quarterly Business Review',
  org: 'MERIDIAN SYSTEMS',
  confidential: 'CONFIDENTIAL — BOARD DISTRIBUTION',
  dataNotice: 'SYNTHETIC RESULTS — DEMONSTRATION ONLY',
  keyboardHint: '← → NAVIGATE · HOME/END',
  code: 'QBR · Q3 FY26',
} as const;

/** The one flagged figure — the single red number in a green KPI row. */
export const NRR_ANOMALY = 'NRR 96% — BELOW 100% FLOOR' as const;
...
export const KPI_SLIDE_NUMBER = SLIDES.findIndex((s) => s.kind === 'kpi') + 1;
...
/* ------------------------------------------------------------------ */
/* Executive summary — three sentences, one of them the anomaly        */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* KPI row — comp.kpi-tile · the anomaly lives here                    */
/* ------------------------------------------------------------------ */
...
export const REVENUE_SERIES: readonly TrendChartSeriesInput[] = [
  {
    id: 'revenue',
    label: 'Revenue ($M)',
    points: [
...
/* ------------------------------------------------------------------ */
/* Segment performance — comp.category-bar-chart ($M vs plan)          */
/* ------------------------------------------------------------------ */

export const SEGMENTS: readonly CategoryBarDatum[] = [
...
export const WINS: readonly Deal[] = [
...
export const LOSSES: readonly Deal[] = [
...
export const RISKS: readonly RiskRow[] = [
  {
...
/* ------------------------------------------------------------------ */
/* Next-quarter priorities — numbered, four                            */
/* ------------------------------------------------------------------ */
...
export const DATA_NOTES: readonly string[] = [

...
`open-design` skill (BORROW route) turns that ID into an adapted copy of the part's code in
...
```
Borrow part <id> using the open-design skill.
```
...
`.claude/skills/open-design/workflows/borrow.md` (one route of the `open-design` skill) — Resolve → Classify → Slice → Adapt → Verify.

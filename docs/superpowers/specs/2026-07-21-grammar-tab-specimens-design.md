# Grammar tab specimens — same content, every grammar
...
bake-off decks (`deck-dgm-*`) already prove the effect for 5 of 16 grammars.
...
- Each grammar gets one **specimen image**: the same opendesign-intro
  ...
  `apps/gallery/public/previews/grammar-<grammarId>.jpg` (1280×800, jpeg
  q75 — same contract as existing previews).
- Specimens are **screenshot-only** for the 11 grammars without a bake-off
...
- Ship gallery changes and all 16 specimens together — the tab lands as a
...
- Uniform framing: 1280×800 viewport, fonts settled, entrance motion
  settled — matching `apps/gallery/scripts/shoot-previews.mjs` output so
...
- Fallback chain (never an empty frame):
  ...
  3. on error again, the existing accent-tinted `PreviewPlate` (already
     ...
     nothing — the same layering contract `PreviewImage` has today).
...
### 5. `GrammarDetail` — the "impact" story, top to bottom

1. **Specimen hero** — the specimen image, large, directly under the title.
2. **"The same design in other grammars"** — a horizontal scroll strip of
   the other 15 grammars' specimens, each a small captioned thumbnail
...
3. **Example templates grid** — replaces the text chips: screenshot
...
4. **Rule lists** — unchanged, below the imagery, as the reference layer.

### 6. Specimen production (the content job)

- For the 11 grammars without a bake-off deck: compose the opendesign-intro
  ...
  cover frame at 1280×800. Working artifacts (composition output, run log)
...
- Extend `shoot-previews.mjs` with a `GRAMMAR_SPECIMENS` map
  (`grammarId → route`) so the 5 live-backed specimens regenerate with the
  normal pipeline; the 11 offline specimens are committed assets
  regenerated by re-running the compose job (documented in the run log).

## Out of scope

- New live worlds, routes, or registry entries for the 11 offline
  specimens; no changes to the Templates tab.
- Per-grammar design tokens or hand-built specimen CSS inside the gallery.
- Extending the full 10-slide interactive bake-off to all 16 grammars
  (possible follow-up, one grammar at a time).

## Testing

- Unit: `GrammarSpecimen` fallback chain (specimen missing → first
  example; both missing → renders nothing over the plate); example-pick
...
- `GrammarDetail`: renders hero, sibling strip (15 links, self excluded),
  example grid links to `detailRoute('experience', id)`.
- Visual sanity: after shooting, eyeball the grammar tab — 16 cards, same
...
- All 16 `grammar-<id>.jpg` files exist in `public/previews/`.

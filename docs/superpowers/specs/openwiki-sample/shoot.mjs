/**
 * Deck-composer skill run (ledger T33) — screenshot every slide of the OpenWiki
 * deck as committed evidence.
 *
 * Loops `?slide=1..N` against the BUILT gallery served by `vite preview`, at
 * 1440x900, waiting for fonts to load and the deck's stable `live-t-minus`
...
async function main() {
  ...
  try {
    for (let n = 1; n <= SLIDE_COUNT; n += 1) {
      ...
      await page.waitForSelector('[data-testid="live-t-minus"]', { timeout: 15_000 });
